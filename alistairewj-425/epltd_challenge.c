...
char *ecglead[21] = {"III", "II", "I", "AVR", "AVL", "AVF",  "V1", "V2", "V3", "V4", "V5", "V6", "V", "MCL1", "MCL2", "MCL3", "MCL4", "MCL5", "MCL6", "MCL", "ECG"};
...
//        }
    ...
    for (leadj=0;leadj<21;leadj++)
	numcharcmp[leadj] = 6;
	

numcharcmp[20] = 3; // just compare to first three letters for ECG

//    for (leadj=0;leadj<21;leadj++)
//	fprintf(stderr,"%d\t%s\t%d\n",leadj,ecglead[leadj],numcharcmp[leadj]);
...
leadi=0;
fprintf(stderr,"%s",s[leadi].desc);
s[leadi].desc="ECG"; // guarantees the code will run on the first lead
fprintf(stderr," %s\n",s[leadi].desc);


    for (leadi = 0; leadi < nsig; leadi++)
      for (leadj = 0; leadj < 21; leadj++)
/* note that the first line after this for loop has to be the if statement
	Alistair 31/07/2013: Had the problem where badly formatted headers (i.e. column 0, column 1, ...) were ignored.
	Changed this to strncmp so that "column" could be compared to, but prints a warning to the user that we're not sure what the lead is.
	*/
        if (strncmp(s[leadi].desc, ecglead[leadj], 6) == 0) { // look for exact string match
          ADCZero = s[leadi].adczero;
          ADCUnit = s[leadi].gain;

	  printf("LEAD NAME: %s", s[leadi].desc);
	  if (strncmp(s[leadi].desc, "column", 6) == 0) {
            printf("***** WARNING, DESCRIPTION NOT SPECIFIC, ENSURE SIGNAL %d IS AN ECG LEAD *****", leadi);
	    sprintf(tempname, "%s%d", "epltd", leadi);
		
	  } else {
	  // Alistair 01/07/2013: Option 1: print the lead name as a part of annotation
	  //  sprintf(tempname, "%s%s", "epltd", ecglead[leadj]);
	  // Alistair 01/07/2013: Option 2: print the signal number as a part of annotation
	   sprintf(tempname, "%s%d", "epltd", leadi);
