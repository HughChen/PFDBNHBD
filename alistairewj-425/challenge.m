function [ qrs_final, ann_jqrs, ann_gqrs, sqi_ecg ] = challenge(recordName, FUSEALG, DELAYALG, opt_input)
%[ beat, sqi ] = challenge(RECORDNAME) detects QRS complexes in the given
...
%% PRE-GAME
% if true, saves detections to annotation files and re-saves the data
SAVE_STUFF = 0;
if nargin<2
FUSEALG = 'sqi';
end
...
if ~isempty(idxECG)
    for m=idxECG
        system(['rdann -r ' recordName 'ep -a epltd' num2str(m-1) ' -x | awk ''{print $1}'' > tmpout']);
        fp = fopen('tmpout','r');
        tmp = textscan(fp,'%s','delimiter','\n');
        tmp = tmp{1};
        tmp = str2double(tmp);
        tmp = round(tmp * fs)+1; % convert 0-1 indexing
		tmp = tmp - 5*fs; % subtract off the initial 5 seconds added in bash
        ann_gqrs{m} = tmp;
    end
end
%% ECG PEAK DETECT
